...
BG_COLOR = (10, 10, 10)
CELL_COLOR = (0, 0, 139)         # Deep blue for cells
PATH_COLOR = (238, 238, 238)     # White-ish path
HIGHLIGHT_COLOR = (255, 220, 0)  # Yellow highlight
WALL_COLOR = (255, 255, 255)     # White walls
UI_BG = (30, 30, 30)
UI_TEXT = (220, 220, 220)
BTN_COLOR = (60, 60, 60)
BTN_ACTIVE = (90, 90, 140)
...
# ---------- Maze Class ----------
class Maze:
    def __init__(self, rows, cols, win_w, win_h, seed=None):
        ...
        self.cells = [[{"walls": {"top": True, "bottom": True, "left": True, "right": True},
                        "color": CELL_COLOR} for _ in range(self.cols)] for _ in range(self.rows)]
    ...
    def draw_cell(self, r, c, color=None):
        ...
        clr = color or self.cells[r][c]["color"]
        pygame.draw.rect(self.surface, clr, pygame.Rect(x, y, self.cell_size, self.cell_size))
        w = self.cells[r][c]["walls"]
        if w["top"]: pygame.draw.line(self.surface, WALL_COLOR, (x, y), (x + self.cell_size, y))
        if w["bottom"]: pygame.draw.line(self.surface, WALL_COLOR, (x, y + self.cell_size), (x + self.cell_size, y + self.cell_size))
        if w["left"]: pygame.draw.line(self.surface, WALL_COLOR, (x, y), (x, y + self.cell_size))
        if w["right"]: pygame.draw.line(self.surface, WALL_COLOR, (x + self.cell_size, y), (x + self.cell_size, y + self.cell_size))
...
def start_carving(seed_r, seed_c):
    def worker():
        updates = maze_cpp.generate(maze.rows, maze.cols, seed_r * maze.cols + seed_c)
        for u in updates: updates_queue.append(u)
...
while running:
            ...
            if maze and maze.generated:
                ...
                if my > ui_height:
                    ...
                    if 0 <= r < maze.rows and 0 <= c < maze.cols:
                        maze.cells[r][c]["color"] = HIGHLIGHT_COLOR
                        maze.draw_cell(r, c)
                        carving_thread = start_carving(r, c)
    ...
    # Full-speed maze generation
    if maze and not maze.generated:
        try:
            while True: next(grid_gen)
        except StopIteration:
            maze.generated = True
            status_text = f"Grid {maze.rows}x{maze.cols} ready. Click a cell to carve."

    # Apply carving updates
    while updates_queue:
        idx, is_wall = updates_queue.popleft()
        r, c = divmod(idx, maze.cols)
        maze.cells[r][c]["color"] = PATH_COLOR if not is_wall else CELL_COLOR
        maze.cells[r][c]["walls"] = {k: is_wall for k in ["top", "bottom", "left", "right"]}
        maze.draw_cell(r, c)
